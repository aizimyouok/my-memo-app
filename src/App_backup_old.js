...
function MemoApp() {
  ...
  const login = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      ...
      // 🔧 토큰을 localStorage에 저장 (보안상 주의: 실제 운영에서는 더 안전한 방법 사용)
    ...
    const restoreToken = async () => {
      try {
        ...
        if (savedToken && savedExpiry) {
          ...
          // 토큰이 아직 유효한지 확인 (10분 여유를 두고)
          if (now < expiryTime - 10 * 60 * 1000) {
            console.log('🔧 저장된 토큰 복원 중...');
            
            // 토큰이 실제로 작동하는지 간단한 테스트
            try {
              ...
              console.log('❌ 저장된 토큰이 유효하지 않음, 삭제합니다.');
              localStorage.removeItem('memo-app-access-token');
              localStorage.removeItem('memo-app-token-expiry');
            }
          } else {
            console.log('⏰ 저장된 토큰이 만료됨, 삭제합니다.');
            localStorage.removeItem('memo-app-access-token');
            localStorage.removeItem('memo-app-token-expiry');
          }
        } else {
        ...
        // 토큰이 없어도 캐시된 데이터는 복원 (오프라인 사용)
        console.log('📂 캐시된 데이터 복원 중...');
        ...
        if (cachedNotebooks && cachedNotebooks.length > 0) {
          console.log('✅ 캐시된 노트북 복원:', cachedNotebooks.length + '개');
          setNotebooks(cachedNotebooks);
          setSelectedNotebookId(cachedNotebooks[0].id);
        } else {
          setSelectedNotebookId('all');
        }
        
        if (cachedMemos && cachedMemos.length > 0) {
          console.log('✅ 캐시된 메모 복원:', cachedMemos.length + '개');
