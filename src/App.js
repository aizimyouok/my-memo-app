...
function SecureMemoApp() {
  ...
  const [viewMode, setViewMode] = useState(() => Storage.load('viewMode') || 'auto'); // 'auto', 'mobile', 'desktop'
  ...
  const handleViewModeChange = useCallback((newViewMode) => {
    ...
    Storage.save('viewMode', newViewMode);
  }, []);

  const getMemosByNotebook = useCallback((notebookId) => {
    if (notebookId === 'all') {
      return appData.memos;
    }
    return appData.memos.filter(memo => memo.notebookId === notebookId);
  }, [appData.memos]);
  ...
  // 📁 노트북 이름 가져오기
  const getNotebookName = useCallback((notebookId) => {
  ...
  // ⭐ 메모 즐겨찾기 토글
  const toggleMemoStar = async (memoId) => {
    try {
      const memoToUpdate = appData.memos.find(m => m.id === memoId);
      if (!memoToUpdate) return;
      
      const updatedMemo = {
        ...memoToUpdate,
        starred: !memoToUpdate.starred,
        updatedAt: new Date().toISOString()
      };
      
      const updatedData = {
        ...appData,
        memos: appData.memos.map(m => m.id === memoId ? updatedMemo : m)
      };
      
      await saveEncryptedData(updatedData);
      setAppData(updatedData);
      
      showToast(updatedMemo.starred ? '즐겨찾기에 추가되었습니다.' : '즐겨찾기에서 제거되었습니다.');
  ...
  const exportData = () => {
    try {
      ...
      link.href = url;
      link.download = `memo-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const importData = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (!importedData.memos || !importedData.notebooks) {
          throw new Error('유효하지 않은 백업 파일입니다.');
        }
        
        const confirmed = window.confirm('현재 데이터를 백업 데이터로 교체하시겠습니까?\n이 작업은 되돌릴 수 없습니다.');
        if (!confirmed) return;
        ...
        const newData = {
          ...
          deletedItems: importedData.deletedItems || []
        };
        
        await saveEncryptedData(newData);
        setAppData(newData);
    ...
    <div style={styles.container} className={isMobile ? 'main-container' : ''}>
      ...
      {isMobile && (
        <div className="mobile-banner">
          📱 모바일 최적화 완료! 터치 친화적인 인터페이스로 이용하세요.
        </div>
      )}
      ...
      {accessToken && isPasswordSet && isUnlocked && (
        <div style={styles.container}>
          {/* 📱 모바일에서는 기존 데스크톱 레이아웃 사용 (CSS로 모바일 최적화) */}
          {/* 왼쪽 패널 */}
